...
def chat():
    ...
    try:
        ...
        is_first_message = request_data.get("isFirstMessage", False)
        ...
        thread_title = None
        if is_first_message:
            # Create a title from the first few words of the user's message (max 50 chars)
            thread_title = user_message[:50] + ("..." if len(user_message) > 50 else "")
        ...
        if thread_title:
            response_data["threadTitle"] = thread_title
